     ...
     {%>
        ...
        <fieldset>
            ...
            <div id="people" class="editor-field">
                ...
                <ul>
                <%
     	foreach (Attendee at in Model.Everyone.Attendees)
     	{%>
                <li>
                    <input type="checkbox" name="SelectedPeopleIds" value="<%=at.person_id%>" <%=Model.SelectedPeopleIds.Contains(at.person_id)
     		                  	? "checked=\"checked\""
     		                  	: ""%> />
                ...
                <div id="mmtChoices" class="editor-field">
                    <dl>
                        <%
     	foreach (MechManiaTeam mmt in Model.Everyone.MechManiaTeams)
     	{%>
                        <dt>
                            <%=String.Format("{0} (Account: {1})",
     		                                Html.Encode(mmt.team_name),
     		                                Html.Encode(mmt.account_name))%></dt>
                        <dd>
                            <ul>
                            <%
     		foreach (Attendee member in mmt.Members)
     		{%>
                                <li>
                ...
                <ul>
                <%
     	foreach (Speaker speaker in Model.Everyone.Speakers)
     	{%>
                <li>
                    <input type="checkbox" name="SelectedPeopleIds" value="<%=speaker.person_id%>" <%=Model.SelectedPeopleIds.Contains(speaker.person_id)
     		                  	? "checked=\"checked\""
     		                  	: ""%> />
                ...
                <ul>
                <%
     	foreach (CompanyPerson sponsor in Model.Everyone.Sponsors)
     	{%>
                <li>
                    <input type="checkbox" name="SelectedPeopleIds" value="<%=sponsor.person_id%>" <%=Model.SelectedPeopleIds.Contains(sponsor.person_id)
     		                  	? "checked=\"checked\""
     		                  	: ""%> />
                ...
                <ul>
                <%
     	foreach (StaffMember sm in Model.Everyone.StaffMembers)
     	{%>
                <li>
                    <input type="checkbox" name="SelectedPeopleIds" value="<%=sm.person_id%>" <%=Model.SelectedPeopleIds.Contains(sm.person_id)
     		                  	? "checked=\"checked\""
     		                  	: ""%> />
                    <%=sm.People.name%>
                </li>
                ...
                <ul>
                <%
     	foreach (Volunteer vol in Model.Everyone.Volunteers)
     	{%>
                <li>
                    <input type="checkbox" name="SelectedPeopleIds" value="<%=vol.person_id%>" <%=Model.SelectedPeopleIds.Contains(vol.person_id)
     		                  	? "checked=\"checked\""
     		                  	: ""%> />
